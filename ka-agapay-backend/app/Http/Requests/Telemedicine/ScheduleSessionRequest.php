...
class ScheduleSessionRequest extends FormRequest
{
    ...
    {
        return [
            'assigned_doctor_id'         => ['required', 'integer', 'exists:users,user_id'],
            'scheduled_date'             => ['required', 'date', 'after_or_equal:today'],
            'scheduled_time'             => ['required', 'date_format:H:i'],
            'estimated_duration_minutes' => ['nullable', 'integer', 'min:5', 'max:120'],
            'session_mode'               => ['sometimes', 'string', 'in:video_call,voice_call,chat,in_app'],
            'bhw_companion_id'           => ['nullable', 'integer', 'exists:users,user_id'],
        ];
    }
}